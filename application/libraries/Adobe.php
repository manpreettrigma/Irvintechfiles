...
class Adobe {
    ...
    public function __construct() {
        $ch = curl_init();
        $this->login = 'ACDeveloper';
        $this->password = 'CLEDeveloper12';
        $this->root_folder = '1336036027';
        $this->template_folder = '1336036027';
        $this->host = 'https://cleseminars.adobeconnect.com/';
     ...
     * @return array
     */
    public function createFolder($name, $description, $parent=NULL, $url=Null) {
     ...
     * @return int
     */
    public function createMeeting($folder_id, $name, $date_begin, $date_end, $url, $template_sco_id = null) {


        $start_date = $this->iso8601($date_begin);
        $end_date = $this->iso8601($date_end);
    ...
    public function getMeetingInfo($meeting_id) {
        ...
        $result = $this->makeRequest('sco-info', array('sco-id' => $meeting_id));
        return $result['sco'];
    }
    ...
    $meeting_id, DateTime $from = null, DateTime $to = null
    ) {
        $request = [
            'sco-id' => $meeting_id,
            'filter-icon' => 'archive'
    ...
    $meeting_id, $group_id, $permission = self::PERMISSION_VIEW
    ) {
        $result = $this->makeRequest('permissions-update', [
    ...
    protected function makeRequest($action, array $params = []) {
        ...
        if (count($errors) > 0 || !isset($data['status']['@attributes']['code']) || $data['status']['@attributes']['code'] !== 'ok') {
