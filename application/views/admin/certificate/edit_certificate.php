                        ...
                        <form class="form-horizontal form-label-left" data-parsley-validate="" id="edit_subject_form" name="edit_subject_form" enctype="multipart/form-data" method="POST" novalidate="">
							...
							<div class="form-group">
									...
									<div class="col-md-10 col-sm-6 col-xs-12">
										<textarea class="form-control" name="certificate[rtme]" rows="3" placeholder=""><?php if(isset( $certificate_data -> rtme )){ echo  $certificate_data -> rtme ; } else{ echo set_value('certificate[workshop_rtme]'); }?></textarea>
										<?php echo form_error('certificate[rtme]', '<div class="error">', '</div>'); ?>
									</div>
								</div>
						
							<div class="form-group">
								
								<label for="product-title" class="control-label col-md-2 col-sm-3 col-xs-12">Select Courses<span class="required">*</span>
                                </label>
								<div class="col-md-9 col-sm-9 col-xs-12">
									<select class="form-control" name="courses_id[]" multiple>
										<?php if(!empty($courses)){?>
											<?php foreach($courses as $course){ ?>
												<option value="<?php echo $course['id']; ?>" <?php if(isset($courseid['courses_id']) && $courseid['courses_id'] == $course['id']){ echo "selected"; } else{ echo set_select("courseid[courses_id]",$course['id']); }?>><?php echo ucfirst($course['title']); ?></option>
											<?php }?>
										<?php }?>												
									</select>
								</div>
                                ...
                                <div class="col-md-10 col-sm-6 col-xs-12">
                                    ...
                                    if($page_image!='') {
                                        ...
                                        <div class="imageOuter">
                                            <a onClick="remove_img('<?php echo $certificate_data ->id; ?>');" href="javascript:void(0) ">
...
CKEDITOR.replace('page_description');
	 function remove_img(kay) {
        $.ajax({url: "<?php echo base_url() ?>admin/certificate/image_delete?id=" + kay, success: function (result) {
                $(".del_img").remove();
				 $("#uploadImg").val('');	
            }});
