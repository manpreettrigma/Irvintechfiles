...
class Payments_pro extends CI_Controller {
    ...
    function Do_authorization() {
        echo "hello" die();
        $DAFields = array(
            'transactionid' => '1', // Required.  The value of the order's transaction ID number returned by PayPal.  
            'amt' => '12', // Required. Must have two decimal places.  Decimal separator must be a period (.) and optional thousands separator must be a comma (,)
            'transactionentity' => '1', // Type of transaction to authorize.  The only allowable value is Order, which means that the transaction represents a customer order that can be fulfilled over 29 days.
            'currencycode' => 'usa', // Three-character currency code.
            'msgsubid' => '2'      // A message ID used for idempotence to uniquely identify a message.
        );
    ...
    function Do_reauthorization() {
        $DRFields = array(
            ...
            'amt' => '10', // Required. Must have two decimal places.  Decimal separator must be a period (.) and optional thousands separator must be a comma (,)
            'currencycode' => 'usa', // Three-character currency code.
            'msgsubid' => '12'      // A message ID used for idempotence to uniquely identify a message.
        );
    ...
    function Set_express_checkout() {
        ...
        $Payments = array();
        $Payment = array(
            'amt' => '12', // Required.  The total cost of the transaction to the customer.  If shipping cost and tax charges are known, include them in this value.  If not, this value should be the current sub-total of the order.
            'currencycode' => '', // A three-character currency code.  Default is USD.
            'itemamt' => '12', // Required if you specify itemized L_AMT fields. Sum of cost of all items in this order.  
            'shippingamt' => '2', // Total shipping costs for this order.  If you specify SHIPPINGAMT you mut also specify a value for ITEMAMT.
            ...
            'sellerid' => '1', // The unique non-changing identifier for the seller at the marketplace site.  This ID is not displayed.
            'sellerusername' => '', // The current name of the seller or business at the marketplace site.  This name may be shown to the buyer.
            'sellerpaypalaccountid' => '1'   // A unique identifier for the merchant.  For parallel payments, this field is required and must contain the Payer ID or the email address of the merchant.
        );
